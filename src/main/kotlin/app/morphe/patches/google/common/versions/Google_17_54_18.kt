...
/**
 * Google App (AGSA) 17.54.18.ve.arm64 - verified hook targets.
 *
 * Obfuscated names below are observations for THIS build only. Keep this file when adding
 * another version; never reuse these names for a different versionName/versionCode.
 */
val google175418 = AppVersionProfile(
    ...
    validation = listOf(
        "2026-09-06: SM-S948Q, microG 7.0.0, user 0 chat and process restart verified (upstream)",
        "2026-09-06: user 150 eligibility logs and user-confirmed Secure Folder success (upstream)",
        "MainActivity branches and appk.k single-instruction replacement verified (upstream)",
        "Ported into this repository without an on-device re-run: see docs/verification.md"
    ),
    hooks = mapOf(
        HookId.WORK_PROFILE_BYPASS to TargetSpec(
            ...
            description = "Both fingerprints resolve the same method; preserve the existing two insertions."
        ),
        ...
        HookId.MAIN_PROCESS_CHECK to TargetSpec(
            ...
            expectedMatches = 1,
            description = "Anchor is in eact.a; patch the provider call inside eact.b to Application.getProcessName."
        ),
        ...
        HookId.GMS_CORE_REDIRECT to TargetSpec(
            ...
            description = "microG RE (app.revanced.android.gms); retain Binder service actions, redirect UI actions. Shared mappings: GmsConstants.kt."
        ),
        HookId.PACKAGE_CLONE_REDIRECT to TargetSpec(
            anchorStrings = listOf("com.google.android.googlequicksearchbox"),
            description = "Observed 156 exact-package substitutions across 144 classes; count depends on other enabled patches."
        ),
        HookId.SPLIT_RESTRICTION_REMOVAL to TargetSpec(
            description = "Remove manifest split requirements; the resource split still needs the package rename and the same signing key."
        ),
